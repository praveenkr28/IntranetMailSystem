...
public class Serv_Sent extends HttpServlet
{
     Statement st;
    Statement st1;
    Connection con;
        ...
        throws ServletException, IOException
    {
        try
        {
            ...
            servletoutputstream.println("{document.f.action='Serv_DeleteCkd1';");
            ...
            servletoutputstream.println("<blink><h1><b><i>Sent Mail Screen of " + uname + " </i></b></h1></blink><br>");
            int i = 0;
            for(ResultSet resultset = st.executeQuery("select mailid from newcompose where mailfrom='" + uname + "' and folder='inbox'"); resultset.next();)
                i++;
            ...
            resultset1.close();*/
            st.close();
            servletoutputstream.println("<br><table border=2 cellpadding=4 cellspacing=5 width=80%><TR bgcolor=#00FFFF><th><font color=red>Status</th><TH><font color=red>To</TH><TH><font color=red>Subject</Th><TH><font color=red>Date</TH></TR>");
            st1 = con.createStatement();
            rs1 = st1.executeQuery("select mailid,mailto,subject,maildate,mailst from newcompose where mailfrom='" + uname + "'");
            int j = 0;
            while(rs1.next()) 
            {
                j++;
                ...
                if(sta == 1)
                {
                    ...
                    servletoutputstream.println("<input type=checkbox name=b" + j + " value=" + m + "></td><td><a href='Serv_SeeMessage1?id=" + m + "'>" + to + "</td></a><td>" + sub + "</td><td>" + date1 + "</td></tr>");
                } else
                {
                    servletoutputstream.println("<tr><td width=6%><input type=checkbox name=b" + j + " value=" + m + "></td><td width=24%><a href='Serv_SeeMessage1?id=" + m + "'>" + to + "</td></a><td width=23%>" + sub + "</td><td width=47%>" + date1 + "</td></tr>");
                }
            }
